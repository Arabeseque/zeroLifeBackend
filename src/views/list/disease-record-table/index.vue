...
// TODO: API
import { DiseaseRecordControllerApi } from '@/service/index';
...
const handleDelClick = async (item: any) => {
  ...
  await api.diseaseRecordDelete([item.id]);
  fetchData();
}
const handleAddBeforeOk = async () => {
  console.log(addForm.value, 'addForm')
  await api.diseaseRecordPost(addForm.value);
  await fetchData();
  return true;
};
const handleBeforeOk = async () => {
  await api.diseaseRecordPost(form.value);
  await fetchData();
  return true;
  // prevent close
  // return false;
...
const fetchData = async (
  ...
  setLoading(true);
  try {
    const { data } = await api.diseaseRecordGet(1, 10)
